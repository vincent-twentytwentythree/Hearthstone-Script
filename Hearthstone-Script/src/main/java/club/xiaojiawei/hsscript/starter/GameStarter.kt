...
//                    游戏刚启动时可能找不到窗口句柄
    ...
    private fun launchGameBySendMessage() {
        ...
        log.info { "click choice of GAME_REGION: ${GAME_REGION}" }
        MouseUtil.leftButtonClick(
            Point(445, rect.bottom - rect.top - 231),
            platformHWND,
            false
        )
        // 299 - 345 - 391
        val regionList = mutableListOf(391, 345, 299)
        if (GAME_REGION < regionList.size) {
            log.info { "start choose GAME_REGION: ${GAME_REGION}" }
            MouseUtil.leftButtonClick(
                Point(470, rect.bottom - rect.top - regionList[GAME_REGION]),
                platformHWND,
                false
            )
            MouseUtil.leftButtonClick(
