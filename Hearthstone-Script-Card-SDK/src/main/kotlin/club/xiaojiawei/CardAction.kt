...
abstract class CardAction(createDefaultAction: Boolean = true) {
    ...
    fun buff(card: Card?, isPause: Boolean = true): CardAction? {
        if (isStop()) return null
        return card?.let {
            val result = execBuff(it)
            if (result) {
                if (isPause) {
                    this.delay()
                } else {
    ...
    protected abstract fun execBuff(card: Card): Boolean
    ...
    abstract class DefaultCardAction : CardAction() {
        ...
        override fun execBuff(card: Card): Boolean {
            return commonAction?.execBuff(card) ?: false
        }
